...
if [[ "$CONDA_DEFAULT_ENV" != "$ENV_NAME" ]]; then
    ...
    if ! conda env list | grep -q "^$ENV_NAME "; then
        ...
        echo "  ./tesseract_python_nb/setup_conda_env.sh"
        exit 1
    fi
...
if [ ! -f "$(pwd)/dependencies.rosinstall" ]; then
    echo "❌ dependencies.rosinstall not found at: $(pwd)/dependencies.rosinstall"
    exit 1
fi
cp "$(pwd)/dependencies.rosinstall" "$WORKSPACE_DIR/src/"
